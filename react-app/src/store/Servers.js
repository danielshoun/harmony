...
export default function reducer(state = initalState, action) {
    switch (action.type) {
        ...
        case JOIN_SERVER: {
            ...
            newState.userServers = [...state.userServers, action.server].sort((a, b) => a.id - b.id);
            const allServersIndex = state.allServers.findIndex(server => server.id === action.server.id);
            newState.allServers = [...state.allServers.slice(0, allServersIndex), action.server, ...state.allServers.slice(allServersIndex + 1, state.allServers.length)];
        ...
        case LEAVE_SERVER: {
            ...
            const allServersIndex = state.allServers.findIndex(server => server.id === action.server.id);
            return {userServers, allServers: [...state.allServers.slice(0, allServersIndex), action.server, ...state.allServers.slice(allServersIndex + 1, state.allServers.length)]};
        }
        case EDIT_SERVER: {
            const newState = {
